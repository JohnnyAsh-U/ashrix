...
func (h *GatewayHandler) CreateGateway(w http.ResponseWriter, r *http.Request) {
	...
	gateway, appErr := h.service.CreateGateway(r.Context(), req.Name, req.IPAddress, req.PublicURL, req.LogToCP)
	if appErr != nil {
...
func (h *GatewayHandler) ReCreateGateway(w http.ResponseWriter, r *http.Request) {
	...
	gateway, appErr := h.service.ReCreateGateway(r.Context(), gatewayID, req.Name, req.IPAddress, req.PublicURL, req.LogToCP)
	if appErr != nil {
		dto.SendError(w, appErr)
		return
	}
...
// @Failure 400 {object} dto.AppError
// @Failure 500 {object} dto.AppError
// @Router /gateways/{id}/drain [put]
func (h *GatewayHandler) DrainGateway(w http.ResponseWriter, r *http.Request) {
	gatewayIDStr := chi.URLParam(r, "id")
	gatewayID, parseErr := uuid.Parse(gatewayIDStr)
	if parseErr != nil {
		dto.SendError(w, dto.NewBadRequestError("Invalid Gateway ID format"))
	...
	gateway, appErr := h.service.DrainGateway(r.Context(), gatewayID)
	if appErr != nil {
		dto.SendError(w, appErr)
		return
	}
