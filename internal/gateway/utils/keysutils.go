...
func SaveKeyAndCertAndBundle(key *ecdsa.PrivateKey, cert,bundle, dataDir, secret, context string) (error) {
	keyPath := filepath.Join(dataDir, "gateway.key.enc")
	certPath := filepath.Join(dataDir, "gateway.crt")
	trustBundlePath := filepath.Join(dataDir, "bundle.crt")

	if err := encryptAndWriteKey(keyPath, key, secret, context); err != nil {
		return fmt.Errorf("invalid PEM in certificate file: %s", certPath)

	}

	if err := writeFile(certPath, []byte(cert), 0644); err != nil {
		return fmt.Errorf("writing public key: %w", err)
	}

	if err :=  writeFile(trustBundlePath, []byte(bundle), 0644); err != nil {
		return fmt.Errorf("writing bundle: %w", err)

	}

	fmt.Printf("  ✓ Saved generated key, cert and bundle\n")
	return nil
}

func LoadKeyAndCertAndBundle(keyPath, certPath, secret, context string) (*ecdsa.PrivateKey, *x509.Certificate, error) {
