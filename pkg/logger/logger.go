...
func (h *ElegantConsoleHandler) WithGroup(name string) slog.Handler {
	// Simplified group implementation: we do not nest/indent groups in elegant console output
	return h
}
...
func initDev(cfg LoggerConfig) error {
	...
	LoggerApp = slog.New(NewElegantConsoleHandler(os.Stdout, opts))
	return nil
}
...
func initProd(cfg LoggerConfig) error {
	...
	// Console logging (Elegant text)
	consoleHandler := NewElegantConsoleHandler(os.Stdout, opts)

	// MultiHandler that logs to both
	LoggerApp = slog.New(&MultiHandler{
